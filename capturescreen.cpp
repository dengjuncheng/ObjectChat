...
void CaptureScreen::captureScreen()
{
    initWindow();
    initStretchRect();
...
void CaptureScreen::loadBackgroundPixmap()
{
    ...
    m_loadPixmap = m_loadPixmap.scaled(width, height); //截取的屏幕图片是屏幕分辨率的两倍，需要缩放，否则会出问题。
    m_screenwidth = width;
...
void CaptureScreen::paintEvent(QPaintEvent *event)
{
    ...
    m_painter.drawPixmap(0, 0, width, height, m_loadPixmap);                       //将背景图片画到窗体上;
...
QString CaptureScreen::saveImg()
{
    ...
    bool result = m_capturePixmap.save("./temp/" + fileName,"PNG");
...
void CaptureScreen::keyPressEvent(QKeyEvent *event)
{
    ...
    {
        QString filePath = saveImg();
        if(filePath != ""){
            emit signalCompleteCature(filePath);
        }
        this->setWindowState(Qt::WindowNoState);
